...
func openData(cfg config.Config) (*store.Store, error) {
	...
	return store.Open(filepath.Join(cfg.DataDir, "kuranotes.sqlite3"))
}
...
func runUsers(cfg config.Config) error {
	...
	fmt.Printf("%-4s %-40s %-6s %s\n", "id", "email", "notes", "created")
	for _, u := range users {
		fmt.Printf("%-4d %-40s %-6d %s\n", u.ID, u.Email, st.CountNotes(u.ID), u.CreatedAt.Format("2006-01-02 15:04"))
	}
	return nil
}
